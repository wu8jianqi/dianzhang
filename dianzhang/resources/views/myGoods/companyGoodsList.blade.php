...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{{asset('static/admin/common/bootstrap/css/bootstrap.css')}}"
          media="all">
            ...
            <div class="layui-col-md2 layui-col-sm4 layui-col-xs6">
                <div class="cmdlist-container">
                    <img src="static/layuiadmin/style/res/template/portrait.png"
                         class="{{'goods_pic'.$value['goods']['goods_id']}}"
                         data-type="{{$value['goods']['goods_pic']}}">
                    <div class="cmdlist-text" style="padding: 6px">
                        <div style="display: flex;justify-content: space-between;font-size: 12px">
                            <span class="{{'goods_name'.$value['goods']['goods_id']}}"
                                  data-type="{{$value['goods']['goods_name']}}">{{$value['goods']['goods_name']}}</span>
                            <span class="{{'is_discount'.$value['goods']['goods_id']}}"
                                  data-type="{{$value['goods']['is_discount']}}">火拼:{{$value['goods']['is_discount']==1?'是':'否'}}</span>
                        </div>
                        <div style="display: flex;justify-content: space-between;font-size: 12px">
                            <span class="{{'present_price'.$value['goods']['goods_id']}}"
                                  data-type="{{$value['goods']['present_price']}}">￥ {{$value['goods']['present_price']/100}}
                                元</span>
                            <span class="{{'stock'.$value['goods']['goods_id']}}"
                        ...
                        <div>
                            <div style="display: flex;justify-content:space-between ">
                                <button type="button" class="layui-btn layui-btn-fluid layui-btn-warm jia " style="border-radius: 12px"
                                        data-type="{{$value['goods']['goods_id']}}"><i
                                ...
                                <input type="number" name="password" required lay-verify="required" placeholder="数量"
                                       ...
                                       class="layui-btn-fluid  {{'jianumber'.$value['goods']['goods_id']}}" {{ isset($redis_data[$value['goods']['goods_id']])? 'value='.$redis_data[$value['goods']['goods_id']]['count'] : ''   }}>
...
<div class="" style="position: fixed ;z-index: 100;bottom: 5px; width: 100%;height: 36px;display: flex;justify-content: space-between">
    ...
    <div style="align-self: center;height:36px;line-height:36px;font-size: 18px;border-top-left-radius: 25px;border-bottom-left-radius: 25px;margin-right: 4px;color: white ;display: flex;justify-content: center"
         class="layui-col-sm3 layui-col-xs3 layui-bg-cyan">数量：<span class="total_count">{{$total['total_count']}}</span>
    </div>
    <div style="align-self: center;height:36px;line-height:36px;font-size: 18px; color: white;display: flex;justify-content: center"
        ...
        总价：<span class="total_price">{{$total['total_price']/100}}</span>元</span>
    </div>
